...
export class VoiceLiveClient {
    ...
    constructor(config: VoiceLiveConfig = {}) {
        // Use external URL if provided in environment, otherwise fallback to local
        const externalHost = process.env.NEXT_PUBLIC_BACKEND_URL;

        let host = 'localhost:3008';
        if (externalHost) {
            // strip http/https
            host = externalHost.replace(/^https?:\/\//, '');
        } else if (typeof window !== 'undefined') {
            host = `${window.location.hostname}:3008`;
        }

        const wsProtocol = externalHost && externalHost.startsWith('https') ? 'wss://' : 'ws://';
        this.serverUrl = config.serverUrl || `${wsProtocol}${host}/voice/live`;

        const queryParams = new URLSearchParams();
        if (config.sessionId) {
            queryParams.append('sessionId', config.sessionId);
        }

        // Pass API Key for WebSocket Auth
    ...
    async connect(): Promise<void> {
        ...
        try {
            ...
            this.websocket.onopen = () => {
                ...
                // Send config message with voice selection and cost controls
