...
import { VoiceLiveClient } from '../lib/VoiceLiveClient.public';
...
export default function VoiceInteraction({ sessionId, onClose, isReadOnly = false }: { sessionId: string, onClose: () => void, isReadOnly?: boolean }) {
    const { getSession, updateSessionStatus, setCallActiveForSession, systemPrompt, agentType, flowData, activeNodeId, setActiveNode } = useSimulationStore();
    ...
    useEffect(() => {
        return () => {
            if (!isTerminating.current) {
                ...
                fetch(`${backendUrl}/api/sessions/${sessionId}/terminate`, {
                    method: 'POST',
                    keepalive: true
